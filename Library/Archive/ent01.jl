function uniqcount(inList::Array{Int64}, inVals::Array{Int64})
# Version 1.0
# Date: 09-05-15 at 12:50:48 AM IST
	ret = zeros(Int64, length(inVals))
	for k in inList
		ret[k] += 1.0
	end
	return ret
...
function ent(x::Array{Int64})

	# Computes the Entropy given a column vector x.
	# x contains categorical values, and are hence non-negative
	# integers. x can take the value 0.
	# The probabilities of the values are determined by
	# the relative count with Laplace smoothing

	# Find the unique values of x
	uniqx = unique(x)
	uniqSize = length(uniqx)
	counts = uniqcount(x,uniqx)
	
	dataSize = length(x) + 2.0
	
	probVals = zeros(Float64,uniqSize)
	# Applying Laplace smoothing
	for k = 1:uniqSize
		probVals[k] = (counts[k]+1.0)/dataSize
		probVals[k] *= log(probVals[k])
	end
	return -1.0*sum(probVals)
end


using Distributions

function kde(x::Array{Float64})
