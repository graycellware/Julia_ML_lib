...
# Version 2.0
# Date: 13-05-15 at 02:45:41 AM IST
include("Causes.jl")
#----------------------------------------------------------------------
#---------------------------------------------------------------------
...
function quinticTrain(data::Array{Int64}, outcomes::Array{Int64},
 ...
 # This to get the edgeLogProbs only
...
function quinticPredict!(data::Array{Int64}, sessionId::ASCIIString, queryId::ASCIIString)
 ...
 =#
 	...
 	inputDataFile = @sprintf("%s_params.csv",sessionId)
 	inputLUTFile = @sprintf("%s_LUT.csv",sessionId)
 	
 	edgeLogProbs = readdlm(inputDataFile,',', Float64)
 	LUT = readdlm(inputLUTFile,',', Int64)
